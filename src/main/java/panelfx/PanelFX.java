...
public class PanelFX extends Application {
                                                                       ...
                                                                       // name=availableCountryDao
    {
        ...
        if ((foundBeans == null) || foundBeans.isEmpty()) {
            throw new UnsatisfiedResolutionException(MessageFormat.format("type: {0}; name:{1}", type.getName(), name));
        }
    ...
    public <T> T getBeanByType(final Class<T> type) {
        ...
        if ((foundBeans == null) || foundBeans.isEmpty()) {
            throw new UnsatisfiedResolutionException(MessageFormat.format("type: {0}", type.getName()));
        }
        final Bean bean = foundBeans.iterator().next();
        final CreationalContext ctx = bm.createCreationalContext(bean); // could
                                                                        // be
    ...
    public void start(final Stage primaryStage) {
        ...
        //    this.weldContainer.instance().select(ApplicationParametersProvider.class).get().setParameters(this.getParameters());
        /**
         ...
         */
        this.weldContainer.event().select(Stage.class, new AnnotationLiteral<StartupScene>() {
        }).fire(primaryStage);

        this.controller = InstanceResolver.getInstance(PanelController.class, this.weldContainer.getBeanManager()).get();
